...
for f=1:nf
  ...
  if any(fidx)
    ...
    if any(outlieridx)
      outliers = strcat(outliers,sprintf('factor:%s;dates:',fName{f}));
      olids=find(outlieridx);
      nolids=length(olids);
      for i=1:nolids
        outliers = strcat(outliers,datestr(tempdata(olids(i))));
        if i<nolids
          outliers = strcat(outliers,',');
        end
      end
      outliers = strcat(outliers,'\r\n');
      tempdata(outlieridx)=[];
...
for f=1:nf
  ...
  if any(vidx)
    ...
    [yfit] = polyval(p,x);
    figure(1);
    plot(x,y,'ko',x,yfit,'r');title(fatorrisco);pause(0.1);
    drawnow
    query = sprintf(['INSERT IGNORE INTO dbkptl.alphabeta ',...
      '(fatorrisco,fator,alpha,beta,r2) VALUES ',...
      '(''%s'',''%s'',%f,%f,%f);'],fatorrisco,fator,p(2),p(1),r2(f));
    h = mysql( 'open', 'localhost','traders', 'kapitalo' );
    mysql(query);
    mysql('close')
    %}
...
%mesas={'KAPITALO 3','KAPITALO 7','KAPITALO 9'};
    ...
    for i=1:length(ufactor)
      ...
      if isKey(fId,ufactor{i})
        ...
        if any(w(plott,factorid))
          ...
          ids = find(ridx(periods{p,2}:periods{p,3},factorid));
          ids = plott(ids);
          securityselection(factorid) = alpha(factorid)*mean(w(ids,factorid));
          temp = cov(w(ids(1:end-1),factorid),r(ids(2:end),ibovid));
          factortiming(factorid) = beta(factorid)*full(temp(1,2));
          riskpremia(factorid) =...
            beta(factorid)*mean(w(ids,factorid))*mean(r(ids,ibovid));
        end
      end
    end
    query = sprintf(['insert ignore into dbkptl.alphabetamesa ',...
      '(periodo,fundo,mesa,securityselection,factortiming,riskpremia) ',...
      'values (''%s'',''%s'',''%s'',%1.6e,%1.6e,%1.6e);'],...
    periods{p,1},fundo,mesa,sum(securityselection),sum(factortiming),sum(riskpremia));
    connected=false;
    while (~connected)
      try
        h = mysql( 'open', 'localhost','traders', 'kapitalo' );
