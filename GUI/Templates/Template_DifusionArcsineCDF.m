function Template_DifusionArcsineCDF(curr_fig, cfighandles, Market)
if ~isempty(cfighandles.symbol)
  ...
  if isempty(cfighandles.ax_time_price.Children)
    ...
    bar(cfighandles.ax_time_price,quotes.openbar(n,end)+maxcenters,cumsum(maxcounts),...
      'c','LineWidth',2);
    hold on
    plot(cfighandles.ax_time_price,quotes.openbar(n,end)+maxcenters,arcsinecdf(2:end),...
      'w','LineWidth',2);
    bar(cfighandles.ax_time_price,quotes.openbar(n,end)+mincenters,-cumsum(mincounts),...
      'm','LineWidth',2);
    plot(cfighandles.ax_time_price,quotes.openbar(n,end)+maxcenters,-arcsinecdf(2:end),...
      'w','LineWidth',2);
