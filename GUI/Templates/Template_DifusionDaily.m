function Template_DifusionDaily(curr_fig, cfighandles, Market)
if ~isempty(cfighandles.symbol)
  ...
  if isempty(cfighandles.ax_time_price.Children)
    ...
    for d=1:ntd-1
      ...
      t = lob:lob+lb-ob;
      logret = quotes.rlogdaily(n,ob:lb);
      c =0.05+ 0.65*d/ntd;
    ...
    cfighandles.ax_time_price.Children(1).YData=...
      quotes.rlogdaily(n,quotes.firstbar(end):quotes.lastbar(n,end));
    firstplot = quotes.firstbar(end);
    firstplot = max(firstplot,1);
    set(cfighandles.ax_time_price, 'Xlim',...
      [firstplot quotes.firstbar(end)+2160]);
    maxp = max(quotes.rlogdaily(n,firstplot:quotes.lastbar(n,end)));
    maxp = max(maxp,max(quotes.rlogdaily(n,:)));
    minp = min(quotes.rlogdaily(n,firstplot:quotes.lastbar(n,end)));
    minp = min(minp,min(quotes.rlogdaily(n,:)));
