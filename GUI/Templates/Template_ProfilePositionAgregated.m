function Template_ProfilePositionAgregated(curr_fig, cfighandles, Market)
if ~isempty(cfighandles.symbol)
  ...
  if isempty(cfighandles.ax_volume_price.Children)
    ...
    for i=1:length(Market.Positions)
      ...
      if ~isempty(strfind(strat.strategy,'Convergence'))
        ...
        if isempty(agregatedContracts)
          agregatedContracts = ...
            position.setpositionprofile;
        else
          agregatedContracts = agregatedContracts+...
            position.setpositionprofile;
        end
      end
    end
    ...
    if ~isempty(position)
      ...
      setmax = max(agregatedContracts([plim(1):plim(2)]));
      setmin = min(agregatedContracts([plim(1):plim(2)]));
  ...
  if isempty(cfighandles.ax_time_volume.Children)
    ...
    stairs(cfighandles.ax_time_volume,...
        [1:quotes.lastbar(n,end)],agregatedContracts,'y','LineWidth',2);
    ...
    for i=1:length(Market.Positions)
      ...
      if ~isempty(strfind(strat.strategy,'Convergence'))
        ...
        if isempty(agregatedContracts)
          agregatedContracts = ...
            symbol.positions.contracts(np,quotes.firstbar(end):quotes.lastbar(n,end)); 
        else
          agregatedContracts = agregatedContracts+...
            symbol.positions.contracts(np,quotes.firstbar(end):quotes.lastbar(n,end)); 
        end
    ...
    if ~isempty(position)
      ...
      cfighandles.ax_time_volume.Children(1).XData=...
        [quotes.firstbar(end):quotes.lastbar(n,end)];
      cfighandles.ax_time_volume.Children(1).YData=agregatedContracts; 
      end
    end
    set(cfighandles.ax_time_volume, 'Xlim', ...
      get(cfighandles.ax_time_price,'Xlim'));
