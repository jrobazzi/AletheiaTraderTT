function Template_ProfileResultAgregated(curr_fig, cfighandles, Market)
if ~isempty(cfighandles.symbol)
  ...
  elseif  quotes.lastbar(n,end)>0
    ...
    xticklabel = datestr(quotes.time(xticks),'dd/mm/yy');
  ...
  elseif quotes.lastbar(n,end)>0
    ...
    if ~isempty(position)
      ...
      agregatedpayoff=[];
      for i=1:length(Market.Positions)
        strat=Market.Positions(i).Strategy;
        if ~isempty(strfind(strat.strategy,'Convergence'))
          np = Market.Positions(i).npos;
          ...
          if isempty(agregatedpayoff)
            agregatedpayoff = payoff;
          else
            agregatedpayoff = agregatedpayoff+payoff;
          end
        end
      end
      
      
      cfighandles.ax_volume_price.Children(4).XData=agregatedpayoff;
      ...
      setmax = max(agregatedpayoff([plim(1):plim(2)]));
      setmin = min(agregatedpayoff([plim(1):plim(2)]));
  ...
  if isempty(cfighandles.ax_time_volume.Children) && quotes.lastbar(n,end)>0
    ...
    agregatedResultCurr=[];
    agregatedResultClosed=[];
    for i=1:length(Market.Positions)
      strat=Market.Positions(i).Strategy;
      if ~isempty(strfind(strat.strategy,'Convergence'))
        np = Market.Positions(i).npos;
        if isempty(agregatedResultCurr)
          agregatedResultCurr = ...
            symbol.positions.resultcurrent(np,1:quotes.lastbar(n,end));
          agregatedResultClosed = ...
            symbol.positions.resultclosed(np,1:quotes.lastbar(n,end));
        else
          agregatedResultCurr = agregatedResultCurr+...
            symbol.positions.resultcurrent(np,1:quotes.lastbar(n,end));
          agregatedResultClosed = agregatedResultClosed+...
            symbol.positions.resultclosed(np,1:quotes.lastbar(n,end));
        end
      end
    end
