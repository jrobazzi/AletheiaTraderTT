function Template_Signals(curr_fig, cfighandles, Market)
if isempty(cfighandles.symbol)
  if isempty(cfighandles.ax_time_price.Children)
    ...
    for s=1:length(Market.Symbols)
      ...
      for sig=1:length(symbol.signals)
        ...
        rlognetaccum=symbol.signals(sig).rlognetaccum(1:lb);
        stairs(cfighandles.ax_time_price,...
          [1:length(rlognetaccum)],rlogaccum,'w');
      end
    end
  ...
  if isempty(cfighandles.ax_time_price.Children)
    ...
    %stairs(cfighandles.ax_time_price,...
    %    [1:quotes.lastbar(n,end)],...
    %    quotes.rlognetaccum(n,1:quotes.lastbar(n,end)),'b');
    %stairs(cfighandles.ax_time_price,...
    %  [1:quotes.lastbar(n,end)],...
    %  -quotes.rlogaccum(n,1:quotes.lastbar(n,end)),'r');
    ...
    if isempty(position)
      ...
      for s=1:length(symbol.signals)
        ...
        rlognetaccum=symbol.signals(s).rlognetaccum(1:lb);
        rlognetaccum = exp(rlognetaccum) - 1;
        stairs(cfighandles.ax_time_price,[1:length(rlognetaccum)],...
          rlognetaccum*100,...
          'LineWidth',2);
      end
      ...
      stairs(cfighandles.ax_time_price,...
        [1:quotes.lastbar(n,end)],...
        symbol.positions.rlognetaccum(np,1:quotes.lastbar(n,end)),...
        'y','LineWidth',2);
      %}
      rlognetaccum = symbol.signals(ns).rlognetaccum(1:quotes.lastbar(n,end))...
      ...
      stairs(cfighandles.ax_time_price,...
        [1:quotes.lastbar(n,end)],rlognetaccum,'y','LineWidth',2);
      stairs(cfighandles.ax_time_price,...
        [1:quotes.lastbar(n,end)],...
  ...
  if isempty(cfighandles.ax_time_volume.Children)
    ...
    for s=1:length(symbol.signals)
      ...
      uw = exp(uw)-1;
      stairs(cfighandles.ax_time_volume,[1:lb],uw*100,...
        'LineWidth',2);
    end
