function Template_SignalsAllocation(curr_fig, cfighandles, Market)
  ...
  if isempty(cfighandles.ax_time_price.Children)
    ...
    if ~isempty(position)
      ...
      stairs(cfighandles.ax_time_price,...
        [1:quotes.lastbar(n,end)],...
        symbol.positions.rlogaccumalloc(np,1:quotes.lastbar(n,end)),...
        'c','LineWidth',2);
      end
  ...
  if isempty(cfighandles.ax_time_volume.Children)
    ...
    if ~isempty(position)
      ...
      if ns>0 
        ...
        if maxalloc>0
          lb = quotes.lastbar(n,end);
          allocratio = symbol.positions.alocation(np,1:lb)./maxalloc;
          stairs(cfighandles.ax_time_volume,[1:lb],allocratio,...
            'c','LineWidth',2);
        end
