function Template_SignalsReturn(curr_fig, cfighandles, Market)
if ~isempty(cfighandles.symbol)
  ...
  if isempty(cfighandles.ax_time_price.Children)
    ...
    nanidx = quotes.openbar(n,2:end-1)-1>=0;
    px(nanidx) = NaN;
    avgpx(nanidx) = NaN;
    upprpx(nanidx) = NaN;
    lowrpx(nanidx) = NaN;
  ...
  if isempty(cfighandles.ax_time_volume.Children)
    ...
    if ~isempty(signal)
      stairs(cfighandles.ax_time_volume,...
        ...
        'w','LineWidth',2);
      stairs(cfighandles.ax_time_volume,...
        [1:lb],signal.rlogaccum(1:lb),'y','LineWidth',2);
      ...
      if ~isempty(position)
        ...
        stairs(cfighandles.ax_time_volume,...
          [1:lb],...
          symbol.positions.rlogaccum(np,1:lb),...
          'c','LineWidth',2);
      else
        stairs(cfighandles.ax_time_volume,...
    ...
    cfighandles.ax_time_volume.Children(2).YData=signal.rlogaccum(fb:lb);
    ...
    if length(signal.rlogaccum)<xl(2)
      xl(2) = length(signal.rlogaccum);
    end
    xl = xl(1):xl(2);
    yl = [min(signal.rlogaccum(xl)) max(signal.rlogaccum(xl))];
    if ~any(yl)
      yl = [0 0.00001];
    end
