function Template_SignalsSharpe(curr_fig, cfighandles, Market)
  ...
  if ~isempty(signal)
    ...
    nzid = find(signal.rlognetaccum,1,'first');
    if ~isempty(nzid)
      firstreturnid = nzid;
      x = 1:lb-nzid+1;
      y = signal.rlognetaccum(1,nzid:lb);
      p = polyfit(x,y,1);
  ...
  if isempty(cfighandles.ax_time_price.Children)
    ...
    if ~isempty(signal)
      stairs(cfighandles.ax_time_price,...
        ...
        signal.rlognetaccum(1,1:quotes.lastbar(n,end)),...
        'y','LineWidth',2);
      stairs(cfighandles.ax_time_price,...
  ...
  if isempty(cfighandles.ax_time_volume.Children)
    ...
    if ~isempty(signal)
      ...
      [1:quotes.lastbar(n,end)],...
      signal.rlogunderwater(1,1:quotes.lastbar(n,end)),...
      'y','LineWidth',2);
    end
