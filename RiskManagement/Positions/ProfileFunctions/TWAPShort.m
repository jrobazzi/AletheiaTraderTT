function TWAPShort( Position )
  ...
  deltacontracts = 995;
  inicontracts = -9995;
  contracts = inicontracts+deltacontracts;
  starttime = datenum(0,0,0,12,10,0);
  finaltime = datenum(0,0,0,18,50,0);
  ...
  if ob > 0
    ...
    if gammaagressor~=0
      if gammaagressor>0 && deltacontracts>0
        Position.reqorders(1,pcol.price) = symbol.bestbid+symbol.ticksize;
      elseif gammaagressor<0 && deltacontracts<0
    ...
    if gammalimit~=0
      ...
      if abs(spread)>symbol.ticksize
        if gammalimit>0 && deltacontracts>0
          Position.reqorders(1,pcol.price) = symbol.bestbid+symbol.ticksize;
        elseif gammalimit<0 && deltacontracts<0
          Position.reqorders(1,pcol.price) = symbol.bestask-symbol.ticksize;
        end
      else
        if gammalimit>0 && deltacontracts>0
