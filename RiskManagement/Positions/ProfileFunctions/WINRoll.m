function WINRoll( Position )
  ...
  first=[];next=[];roll=[];
  sfirst='WINM17';snext='WINQ17';sroll='WI1M17Q17';
  fid=0;nid=0;rid=0;
  main = Position.Main;
  ...
  for i=1:length(first.Positions)
    if strcmp(first.Positions(i).Strategy.strategy,'WINRoll')
      firstpos = first.Positions(i);
      break;
    end
  end
  for i=1:length(next.Positions)
    if strcmp(next.Positions(i).Strategy.strategy,'WINRoll')
