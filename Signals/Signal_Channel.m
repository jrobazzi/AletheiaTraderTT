function signal = Signal_Channel(symbol,signal)
  if ~signal.init
    ...
    for t=signal.lastbar:lb
      ...
      if t>=finalchannelid && t<=closepositionid
        ...
        if maxp>channelup
          ...
          signal.slippage(t) = ...
            log((channelup+symbol.ticksize)/lstp)*signal.gamma(t);
        elseif minp<channeldn
          signal.gamma(t) = -1 - signal.delta(t-1);
          signal.slippage(t) = ...
            log((channeldn-symbol.ticksize)/lstp)*signal.gamma(t);
        elseif signal.delta(t-1)>0 && minp<=channelmid 
          signal.gamma(t) = 0 - signal.delta(t-1);
          signal.slippage(t) = log(channelmid/lstp)*signal.gamma(t);
...
function signal = InitSignal(symbol,signal)
  ...
  %% TRADEDATES LOOP
  starttime = round((datenum(0,0,0,0,10,0)-quotes.dt)/quotes.dt);
  stoptime = round((datenum(0,0,0,8,50,0)-quotes.dt)/quotes.dt);
...
function signal = Integrator(symbol,signal,quotes,n,t,fb,ob,lb)
