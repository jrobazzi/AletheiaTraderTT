function signal = Signal_ChannelGamma10TWAP1Early(symbol,signal)
  if ~signal.init
...
function signal = InitSignal(symbol,signal)
  ...
  tdot = tic;
  %% TRADEDATES LOOP
  starttime = round((datenum(0,0,0,0,1,0)-quotes.dt)/quotes.dt);
  stoptime = round((datenum(0,0,0,4,0,0)-quotes.dt)/quotes.dt);
  finaltime = round((datenum(0,0,0,4,50,0)-quotes.dt)/quotes.dt);
