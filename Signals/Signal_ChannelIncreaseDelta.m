function signal = Signal_ChannelIncreaseDelta(symbol,signal)
  if ~signal.init
...
function signal = InitSignal(symbol,signal)
  ...
  stopincreasetime = round((datenum(0,0,0,4,0,0)-quotes.dt)/quotes.dt);
  for d=1:length(quotes.tradedates)
    ...
    stopincreaseid =  ob + stopincreasetime;
    channelup = [];
    channeldn = [];
    %% INTRADAY LOOP
    for t=signal.lastbar:lb
      ...
      minp = quotes.min(n,t);
      %-----------------------------GAMMA---------------------------------
      deltatarget = 0.5 + 0.5*((t-ob)/(stopincreaseid-ob));
