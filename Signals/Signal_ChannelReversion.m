function signal = Signal_ChannelReversion(symbol,signal)
  if ~signal.init
...
function signal = InitSignal(symbol,signal)
  ...
  allocation = zeros(size(quotes.tradedates));
  zscore = zeros(size(quotes.tradedates));
  minzscore = 1; maxzscore = 6;maxgamma=0.02;
  for d=1:length(quotes.tradedates)
    ...
    if ob>0
      ...
      closepositionid = lb -240;
      finalpositionid = lb -20;
      ...
      for t=signal.lastbar:lb
        ...
        %-----------------------------GAMMA---------------------------------
        if t==ob
          zscore(d) = symbol.filters.resid(t,end)/symbol.filters.sig(d,end);
          if abs(zscore(d))>=minzscore
            allocation(d) = abs(zscore(d))/maxzscore;
            allocation(d) = min(allocation(d),1);
            allocation(d) = max(allocation(d),0);
          else
            allocation(d) = 0;
          end
        elseif t==lb
          signal.gamma(t) = 0 - signal.delta(t-1);
        elseif t>=finalchannelid && t<=closepositionid && t<lb
          channelup=max(quotes.max(n,ob:finalchannelid));
          channeldn=min(quotes.min(n,ob:finalchannelid));
          channelmid= (channelup+channeldn)/2;
          channelmid= round(channelmid/symbol.ticksize)*symbol.ticksize;
          if maxp>channelup && zscore(d)<0
            signal.gamma(t) = allocation(d)*fadePos - signal.delta(t-1);
          elseif minp<channeldn && zscore(d)>0
            signal.gamma(t) = -allocation(d)*fadePos - signal.delta(t-1);
          elseif signal.delta(t-1)>0 && minp<=channelmid 
          ...
          %rate limiter
          if signal.gamma(t)>maxgamma
            signal.gamma(t)=maxgamma;
          elseif signal.gamma(t)<-maxgamma
            signal.gamma(t)=-maxgamma;
          end
        else
          signal.gamma(t) = 0 - signal.delta(t-1);
