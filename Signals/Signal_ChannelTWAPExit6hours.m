function signal = Signal_ChannelTWAPExit6hours(symbol,signal)
  if ~signal.init
...
function signal = InitSignal(symbol,signal)
  ...
  starttwaptime = round((datenum(0,0,0,1,0,0)-quotes.dt)/quotes.dt);
  for d=1:length(quotes.tradedates)
    ...
    for t=signal.lastbar:lb
      ...
      if t<starttwapid
        ...
        deltatarget = (lb-t)/(lb-starttwapid);
      end
      ...
      elseif t>=finalchannelid
        if maxp>channelup
          signal.gamma(t) = 1*deltatarget - signal.delta(t-1);
        elseif minp<channeldn
          signal.gamma(t) = -1*deltatarget - signal.delta(t-1);
