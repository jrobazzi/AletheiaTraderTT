function signal = Signal_ConvergenceId(symbol,signal)
  if ~signal.init
...
function signal = InitSignal(symbol,signal)
  ...
  tdot = tic;
  %parameters
  sigref = 4;spread = 3;tauMA = 1080*1; slip=2;
      ...
      for t=signal.lastbar:lb
        ...
        if t>=ob && t<lb
          ...
          elseif lstp<=avgp
            ...
            if inLongRef > ldelta
              signal.gamma(t) = fadePos*inLongRef - ldelta;
            else
              outLongRef = ...
              ...
              if outLongRef<ldelta
                signal.gamma(t) = fadePos*outLongRef - ldelta;
              else
                signal.gamma(t) = fadePos*ldelta - ldelta;
              end
            end
          elseif lstp>=avgp
            %short
            ...
            if inShortRef < ldelta
              signal.gamma(t) = fadePos*inShortRef - ldelta;
            else
              outShortRef = ...
              ...
              if outShortRef>ldelta
                signal.gamma(t) = fadePos*outShortRef - ldelta;
              else
                signal.gamma(t) = fadePos*ldelta - ldelta;
              end
            end
