function signal = Signal_Convergence(symbol,signal)
  if ~signal.init
...
function signal = InitSignal(symbol,signal)
  ...
  tdot = tic;
  %parameters
  sigref = 2;spread = 2;tauMA = 540*1; slip=2;
  nma = find(symbol.filters.tau == tauMA);
  sigMA = symbol.filters.sig(nma);
  %% TRADEDATES LOOP
  for d=1:length(quotes.tradedates)
    ...
    if ob>0
      band = max(symbol.filters.residUp(d,nma),...
        abs(symbol.filters.residDn(d,nma)));
      if band == 0 
        band = 0.01;
      end
      longsigref = band/sigMA;
      longsigref = sigref;
      inLong=-longsigref; outLong=0; inLongExp = spread; outLongExp = 1/inLongExp;
      shortsigref = band/sigMA;
      shortsigref = sigref;
      inShort=shortsigref; outShort=0; inShortExp = spread; outShortExp=1/inShortExp;
      %% INTRADAY LOOP
      for t=signal.lastbar:lb
        lstp = quotes.close(n,t);
        maxp = quotes.max(n,t);
        ...
        if t>=ob && t<=lb
          ...
          elseif lstp<=avgp
            %long
            xLong = (lstp-avgp)/(sigMA*longsigref*avgp);
            if xLong<-longsigref
              xLong=-longsigref;
            end
            inLongRef = ...
          ...
          elseif lstp>=avgp
            %short
            xShort = (lstp-avgp)/(sigMA*shortsigref*avgp);
