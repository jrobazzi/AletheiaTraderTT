function signal = Signal_Convergence_1080_4_3(symbol,signal)
  if ~signal.init
  ...
  tauMA = 1080*1;sigref = 4;spread = 3; slip=2;
  nma = find(symbol.filters.tau == tauMA);
  %nma = size(symbol.filters.avg,2);
  ...
  if ob>0 && lb>0
    ...
    for t=signal.lastbar:lb
     ...
     %% --------------------------INTEGRATORS---------------------------
        ...
        %minimum gamma
        if abs(signal.delta(t-1))<0.001 && ...
            abs(signal.gamma(t))<0.001 && signal.gammadir(t)==-1
          %allow close small position
          signal.slippage(t) = abs(signal.gamma(t))*...
            -log((quotes.close(n,t)+slip*symbol.ticksize)/quotes.close(n,t));
        elseif abs(signal.gamma(t))<0.001 && signal.gammadir(t)>0
          %dont allow small trades
          signal.gamma(t) = 0;
          signal.slippage(t) = 0;
        else
          signal.slippage(t) = abs(signal.gamma(t))*...
