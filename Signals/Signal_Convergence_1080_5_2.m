function signal = Signal_Convergence_1080_5_2(symbol,signal)
  if ~signal.init
  ...
  tauMA = 1080*1;sigref = 5;spread = 2; slip=2;
  ...
  for d=1:length(quotes.tradedates)
    ...
    if ob>0 && d>=(tauMA*30/540)
      ...
      for t=signal.lastbar:lb
        ...
        %minimum gamma
        if abs(signal.delta(t-1))<0.005 && ...
            abs(signal.gamma(t))<0.005 && signal.gammadir(t)==-1
          %allow close small position
          signal.slippage(t) = abs(signal.gamma(t))*...
            -log((quotes.close(n,t)+slip*symbol.ticksize)/quotes.close(n,t));
        elseif abs(signal.gamma(t))<0.005 && signal.gammadir(t)>0
