function signal = Signal_Divergence(symbol,signal)
  if ~signal.init
...
function signal = InitSignal(symbol,signal)
  ...
  tdot = tic;
  %parameters
  sigref = 0.5;tauMA = 5400;maxgamma=0.005;
  ...
  for d=1:length(quotes.tradedates)
    ...
    if ob>0
      ...
      for t=signal.lastbar:lb
        ...
        fastavgp = symbol.filters.avg(t,1);
        %-----------------------------GAMMA---------------------------------
        if t>=ob && t<=lb
          deltaref = (fastavgp-avgp)/(sigMA*sigref*avgp);
          deltaref = max(-1,deltaref);
          deltaref = min(1,deltaref);
          signal.gamma(t) = deltaref-signal.delta(t-1);
          if abs(signal.gamma(t))>maxgamma
            signal.gamma(t) = sign(signal.gamma(t))*maxgamma;
          end
        end
        %------------------------------------------------------------------
        %% --------------------------INTEGRATORS---------------------------
