function signal = Signal_LongFlyMA300(symbol,signal)
  if ~signal.init
  ...
  %% INTRADAY LOOP
    for t=signal.lastbar:lb
      ...
      if d>=100 && t>ob+10 && t<=closepositionid
...
function signal = InitSignal(symbol,signal)
  ...
  n = symbol.n;
  periodMAmin = 300;
  tauMA = 1-exp(-(2*pi*15^2)/(periodMAmin*3600));
  MA = zeros(size(quotes.rlog(n,:)));
  ...
  for d=1:length(quotes.tradedates)
    ...
    %% INTRADAY LOOP
    for t=signal.lastbar:lb
      %MOVING AVERAGE and POSITION FADE OUT
      if t==ob
        MA(t)=quotes.close(n,ob);
      elseif quotes.volume(n,t)~=0
        MA(t)=MA(t-1)*(1-tauMA)+quotes.close(n,t)*tauMA;
      else
        MA(t)=MA(t-1);
      end
      ...
      if d>=100
        ...
        elseif t>startid && t<=finalpositionid
          cShift = round((MA(t)-quotes.close(n,ob))/symbol.ticksize);
          rlogid = round(lstp/symbol.ticksize);
          dref = deltaref(rlogid-cShift);
          dref = dref*fadePos;
          derr = dref-signal.delta(t-1);
